...
public class AuroraSpaceHomePage {
  ...
  public NextEvents getNextEvents() {
    ...
    if (displayEvents) {
      ...
      return look.getNextEvents(allowedAppIds);
    }
    return null;
  }
  ...
  private Shortcut getShortcut(String name, String nb) {
    ...
    if (StringUtil.isDefined(url)) {
      ...
      String alt = getFieldValue(name+"Alt"+nb);
        ...
        if (fieldValue != null) {
          ...
          if (StringUtil.isDefined(attachmentId)) {
            if (attachmentId.startsWith("/")) {
              ...
              SimpleDocument attachment = AttachmentService.get()
                  .searchDocumentById(new SimpleDocumentPK(attachmentId, getBackOfficeApp().getId()), null);
              if (attachment != null) {
  ...
  private String getFieldValue(String name) {
    try {
      return data.getField(name).getStringValue();
    } catch (Exception e) {
      SilverLogger.getLogger(this).error(e);
      return "";
    }
